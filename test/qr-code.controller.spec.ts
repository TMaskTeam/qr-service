...
import { ScanQrCodeUseCase } from '../src/application/use-cases/scan-qr-code.use-case';
...
describe('QrCodeController', () => {
	it('returns token payload and QR image data URL', async () => {
		...
		const controller = new QrCodeController(
			{ execute: executeMock } as unknown as DailyQrCodeUseCase,
			{ execute: jest.fn() } as unknown as ScanQrCodeUseCase,
		);
